...
@register('miniImageNet_spec')
class miniImageNet_spec(Dataset):
    ...
    def __getitem__(self, i):
        ...
        img = self.transform(self.data[i])    # 把一张原始图像经过数据增强 or 默认的变换 得到预处理后的图像

        # 将一张图像随机裁剪成 patch_num 个图像
        if self.localPatchCrop != None:
            image = []
            for i in range(self.patch_num):
                image.append(self.localPatchCrop(img))
            image = torch.stack(image)
        else:
            image = img

        return global_image, image, self.label[i]
